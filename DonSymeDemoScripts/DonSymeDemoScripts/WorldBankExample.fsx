...
countries1.[0].Indicators.``Adolescent fertility rate (births per 1,000 women ages 15-19)``.TryGetValueAt(2005).Value

let pointdata = 
    [ for country in countries1 ->
          let y = country.Indicators.``Adolescent fertility rate (births per 1,000 women ages 15-19)``.TryGetValueAt(2005).Value
          let x = country.Indicators.``Primary completion rate, female (% of relevant age group)``.TryGetValueAt(2005).Value
          x,y ]
...
let popAt year =
 ((query { for c in data.Countries  do 
           let pop = c.Indicators.``Population, total``.TryGetValueAt(year).Value 
